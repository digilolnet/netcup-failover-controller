...
func TestReconcile_MissingAnnotation(t *testing.T) {
	api := &mockAPI{}
	r := newReconciler(t, api, testFIP("group-a", "198.51.100.1/32"), readyNode("node-a", ""), testSecret())

	if _, err := reconcileOnce(t, r, "group-a"); err != nil {
		t.Fatalf("expected no error (annotating triggers a node event), got: %v", err)
	}
	if cond := routedCondition(t, r, "group-a"); cond.Reason != netcupv1alpha1.ReasonMissingAnnotation {
		t.Errorf("condition reason = %s, want %s", cond.Reason, netcupv1alpha1.ReasonMissingAnnotation)
	}
	if len(api.routeCalls) != 0 {
		t.Errorf("expected no route calls, got: %+v", api.routeCalls)
	}
...
func TestReconcile_ServerNotFound(t *testing.T) {
	api := &mockAPI{servers: []scp.ServerListMinimal{server(t, 101, "srv-other")}}
	r := newReconciler(t, api, testFIP("group-a", "198.51.100.1/32"), readyNode("node-a", "srv-a"), testSecret())

	if _, err := reconcileOnce(t, r, "group-a"); err != nil {
		t.Fatalf("expected no error (fixing the annotation triggers a node event), got: %v", err)
	}
	if cond := routedCondition(t, r, "group-a"); cond.Reason != netcupv1alpha1.ReasonServerNotFound {
		t.Errorf("condition reason = %s, want %s", cond.Reason, netcupv1alpha1.ReasonServerNotFound)
	}
}
