...
func TestSession_ServerIDByName(t *testing.T) {
	tests := []struct {
		name    string
		api     *stubAPI
		lookup  string
		...
		{
			name:   "found",
			api:    &stubAPI{servers: []scp.ServerListMinimal{serverFixture(t, 101, "srv-a"), serverFixture(t, 102, "srv-b")}},
			lookup: "srv-b",
			want:   102,
		},
		{
			name:    "not found",
			api:     &stubAPI{servers: []scp.ServerListMinimal{serverFixture(t, 101, "srv-a")}},
			lookup:  "srv-missing",
		...
		},
		{
			...
			lookup:  "srv-a",
			wantErr: errors.New("boom"), // matched by message below
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := testSession(tt.api).ServerIDByName(t.Context(), tt.lookup)
			if tt.wantErr != nil {
				if err == nil || !strings.Contains(err.Error(), tt.wantErr.Error()) {
					t.Fatalf("error = %v, want containing %q", err, tt.wantErr)
				}
				if errors.Is(tt.wantErr, ErrServerNotFound) && !errors.Is(err, ErrServerNotFound) {
					t.Fatalf("error = %v, want ErrServerNotFound in chain", err)
				}
				return
			}
			if err != nil {
			...
			if got != tt.want {
				t.Errorf("ServerIDByName = %d, want %d", got, tt.want)
			}
		})
	}
}
