...
// ErrServerNotFound reports that no SCP server in the account matches the
// requested name.
var ErrServerNotFound = errors.New("server not found")

// ServerIDByName maps an SCP server name to the numeric server ID the REST
// API operates on.
func (s *Session) ServerIDByName(ctx context.Context, name string) (int32, error) {
	servers, err := s.API.ListServers(ctx, &scp.ListServersOptions{Name: &name})
	if err != nil {
		return 0, fmt.Errorf("listing servers: %w", err)
	}
	for _, srv := range servers {
		if srv.Name != nil && *srv.Name == name && srv.Id != nil {
			return *srv.Id, nil
		}
	}
	return 0, fmt.Errorf("%w: no SCP server named %q", ErrServerNotFound, name)
}
